...
import com.deckerpw.flateditor.lang.java.buildpath.JarLibraryInfo;
...
public class JarManager {
            ...
            for (JarReader jar : classFileSources) {
                ...
                if (classFiles != null) {
                    for (ClassFile cf : classFiles) {
                        if (com.deckerpw.flateditor.lang.java.classreader.Util.isPublic(cf.getAccessFlags())) {
	...
	public boolean setJdk(File jdkHome) throws IOException {
		if (jdkHome==null) { clearJdk(); return true; }
		LibraryInfo info = LibraryInfo.getJreJarInfo(jdkHome);
		if (info==null) throw new IOException("Not a valid JDK: " + jdkHome);
		return setJdk(info);
	}

	public boolean setJdk(java.nio.file.Path jdkHome) throws IOException {
		return setJdk(jdkHome==null?null:jdkHome.toFile());
	}

	public boolean setJdk(LibraryInfo jdkInfo) throws IOException {
		clearJdk();
		if (jdkInfo==null) return true;
		classFileSources.add(0, new JarReader(jdkInfo));
		return true;
	}

	public void clearJdk() {
		if (!classFileSources.isEmpty()) {
			LibraryInfo first = classFileSources.get(0).getLibraryInfo();
			if (first instanceof com.deckerpw.flateditor.lang.java.buildpath.Jdk9LibraryInfo || isJdkInfo(first)) {
				classFileSources.remove(0);
			} else if (first.getLocationAsString()!=null && first.getLocationAsString().contains("rt.jar")) {
				classFileSources.remove(0);
			} else if (first.getLocationAsString()!=null && first.getLocationAsString().endsWith("jmods") || first.getLocationAsString().isEmpty()) {
				classFileSources.remove(0);
			}
		}
	}

	private boolean isJdkInfo(LibraryInfo info) {
		String loc = info.getLocationAsString();
		return loc!=null && (loc.contains("jmods") || loc.contains("rt.jar") || loc.contains("classes.jar"));
	}

	public boolean setProjectLibs(List<LibraryInfo> libs) throws IOException {
		clearProjectLibs();
		if (libs!=null) for (LibraryInfo li : libs) addClassFileSource(li);
		return true;
	}

	public void clearProjectLibs() {
		LibraryInfo jdk = null;
		if (!classFileSources.isEmpty()) {
			LibraryInfo first = classFileSources.get(0).getLibraryInfo();
			if (isJdkInfo(first) || first instanceof com.deckerpw.flateditor.lang.java.buildpath.Jdk9LibraryInfo) jdk = first;
		}
		classFileSources.clear();
		if (jdk!=null) try { classFileSources.add(new JarReader(jdk)); } catch (IOException e) { e.printStackTrace(); }
	}

	public boolean setBuildPath(BuildPath buildPath) throws IOException {
		clearClassFileSources();
		if (buildPath==null) return true;
		if (buildPath.getJdk()!=null) addClassFileSource(buildPath.getJdk());
		for (LibraryInfo li : buildPath.getProjectLibs()) addClassFileSource(li);
		return true;
	}
	...
	public BuildPath getBuildPath() {
		...
		for (int i=0;i<classFileSources.size();i++) {
			LibraryInfo info = classFileSources.get(i).getLibraryInfo();
			if (i==0 && (isJdkInfo(info) || info instanceof com.deckerpw.flateditor.lang.java.buildpath.Jdk9LibraryInfo)) jdk = info;
			else libs.add(info);
		}
		return new BuildPath(jdk, libs);
	}

	public boolean addLibrary(File file) throws IOException {
		return addClassFileSource(LibraryInfo.fromFile(file));
	}

	public boolean addLibrary(java.nio.file.Path path) throws IOException {
		return addLibrary(path.toFile());
	}

	public boolean addLibrary(File file, File source) throws IOException {
		return addClassFileSource(LibraryInfo.fromFile(file, source));
	}
