...
public class Jdk9LibraryInfo extends LibraryInfo {
	...
	public int compareTo(LibraryInfo info) {
		...
		Jdk9LibraryInfo other = (Jdk9LibraryInfo)info;
		if (jmodFiles.length != other.jmodFiles.length) {
			return jmodFiles.length - other.jmodFiles.length;
		}
		java.util.Arrays.sort(jmodFiles);
		java.util.Arrays.sort(other.jmodFiles);
		for (int i=0;i<jmodFiles.length;i++) {
			int c = jmodFiles[i].compareTo(other.jmodFiles[i]);
			if (c!=0) return c;
		}
		return 0;
	}
	...
	public String getLocationAsString() {
		File home = getJreHome();
		return home==null ? "" : home.getAbsolutePath();
	}
