...
import com.deckerpw.flateditor.lang.AbstractLanguageSupport;
import com.deckerpw.flateditor.lang.GoToMemberAction;
...
import com.deckerpw.flateditor.lang.java.rjc.ast.Package;
import com.deckerpw.flateditor.lang.java.tree.JavaOutlineTree;
...
public class JavaLanguageSupport extends AbstractLanguageSupport {
	...
	private final Map<RSyntaxTextArea, JarManager> perEditorManagers = new HashMap<>();
	private final Map<RSyntaxTextArea, BuildPath> perEditorBuildPaths = new HashMap<>();

	/**
	 * Client property installed on text areas that points to a listener.
	 */
	private static final String PROPERTY_LISTENER =
		"com.deckerpw.flateditor.lang.java.JavaLanguageSupport.Listener";
	...
	public JarManager getJarManager(RSyntaxTextArea textArea) {
		JarManager jm = perEditorManagers.get(textArea);
		return jm!=null ? jm : jarManager;
	}

	public BuildPath getBuildPath(RSyntaxTextArea textArea) {
		return perEditorBuildPaths.get(textArea);
	}
	...
	public void setBuildPath(RSyntaxTextArea textArea, BuildPath buildPath) {
		try {
			...
			if (jm==null) {
				jm = new JarManager();
				perEditorManagers.put(textArea, jm);
				JavaCompletionProvider p = getCompletionProvider(textArea);
				if (p!=null) p.setJarManager(jm);
			}
			jm.setBuildPath(buildPath);
			if (buildPath!=null) perEditorBuildPaths.put(textArea, buildPath);
			else perEditorBuildPaths.remove(textArea);
		} catch (Exception e) { e.printStackTrace(); }
	}

	public void setJdk(RSyntaxTextArea textArea, java.io.File jdkHome) {
		try {
			JarManager jm = perEditorManagers.get(textArea);
			if (jm==null) { jm = new JarManager(); perEditorManagers.put(textArea, jm); }
			if (jdkHome==null) jm.clearJdk();
			else jm.setJdk(jdkHome);
			JavaCompletionProvider p = getCompletionProvider(textArea);
			if (p!=null) p.setJarManager(jm);
			BuildPath old = perEditorBuildPaths.get(textArea);
			LibraryInfo jdk = jdkHome==null? null : LibraryInfo.getJreJarInfo(jdkHome);
			java.util.List<LibraryInfo> libs = old==null? java.util.Collections.emptyList() : old.getProjectLibs();
			perEditorBuildPaths.put(textArea, new BuildPath(jdk, libs));
		} catch (Exception e) { e.printStackTrace(); }
	}

	public void setJdk(RSyntaxTextArea textArea, java.nio.file.Path jdkHome) {
		setJdk(textArea, jdkHome==null?null:jdkHome.toFile());
	}

	public void setProjectLibs(RSyntaxTextArea textArea, java.util.List<LibraryInfo> libs) {
		try {
			JarManager jm = perEditorManagers.get(textArea);
			if (jm==null) { jm = new JarManager(); perEditorManagers.put(textArea, jm); }
			jm.setProjectLibs(libs);
			JavaCompletionProvider p = getCompletionProvider(textArea);
			if (p!=null) p.setJarManager(jm);
			BuildPath old = perEditorBuildPaths.get(textArea);
			LibraryInfo jdk = old==null? null : old.getJdk();
			perEditorBuildPaths.put(textArea, new BuildPath(jdk, libs));
		} catch (Exception e) { e.printStackTrace(); }
	}

	public void addJar(RSyntaxTextArea textArea, java.io.File jar) {
		try {
			JarManager jm = getJarManager(textArea);
			jm.addLibrary(jar);
		} catch (Exception e) { e.printStackTrace(); }
	}

	public void setBuildPath(BuildPath buildPath) {
		try { jarManager.setBuildPath(buildPath); } catch (Exception e) { e.printStackTrace(); }
	}
	...
	public void install(RSyntaxTextArea textArea, BuildPath buildPath) {
		JarManager jm = new JarManager();
		try { jm.setBuildPath(buildPath); } catch (Exception e) { e.printStackTrace(); }
		perEditorManagers.put(textArea, jm);
		perEditorBuildPaths.put(textArea, buildPath);
		installWithManager(textArea, jm);
	}

	@Override
	public void install(RSyntaxTextArea textArea) {
		JarManager jm = perEditorManagers.get(textArea);
		if (jm==null) jm = jarManager;
		installWithManager(textArea, jm);
	}

	private void installWithManager(RSyntaxTextArea textArea, JarManager jm) {
		JavaCompletionProvider p = new JavaCompletionProvider(jm);
		...
		ac.setExternalURLHandler(new JavadocUrlHandler(this));
	...
	public void uninstall(RSyntaxTextArea textArea) {
		...
		perEditorManagers.remove(textArea);
		perEditorBuildPaths.remove(textArea);
